...
const ProfileFields = ({ label, ...props }) => {
    ...
    return (
      <div className='form-group px-5 mt-3'>
        {label && (
          <label
            htmlFor={field.name}
            className='block text-[18px] text-[#00000] line-height-[21.09px]'
          >
            {label}
          </label>
        )}
        <input
          className={`rounded-[3px] lg:w-[407px] lg:h-[56px] h-[40px] w-full  mt-2  p-2 border border-[#BFBFBF] focus:ring-blue-500 focus:border-blue-500 focus:outline-none     dark:focus:ring-blue-500 dark:focus:border-blue-500 ${
            meta.touched &&
            meta.error &&
            'border-1 border-red-500 error-form error-form'
          }`}
